...
function DogProvider(props) {
  const [state, dispatch] = React.useReducer(dogReducer, { dogName: '' });
    return (
        <DogContext.Provider value={[state, dispatch]} {...props} />
    )
};
...
function DogNameInput() {
  ...
  const [{dogName}, dispatch] = useDogState(); // extra 1

  function handleChange(event) {
    const newDogName = event.target.value
    // 🐨 change this to call your state setter that you get from useState
    dispatch({type: 'TYPED_IN_DOG_INPUT', dogName: newDogName}) // extra 1
